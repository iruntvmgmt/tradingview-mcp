...
`_validate_bar_budget()` runs at config load time:
...
- Estimates bars per window: `window_minutes / bar_minutes`
- Rejects windows exceeding `intraday_bar_limit`

**Example pass:**
```
15m, 31-day window → ~2,976 bars → under 5,000 limit → OK
```

**Example fail:**
```
WindowGuardError: train window needs approximately 17,280 bars on 15m,
but TradingView tier free is configured for 5,000 intraday bars. Use a
shorter window, higher timeframe, or higher TradingView tier.
```
...
| `test_feasible_intraday_window_passes` | 15m, ~31 days, ~2,976 bars → passes |
...
| `experiment_config.json` | Added `timeframe`, `tradingview_tier` section |
| `tests/test_experiment_controller.py` | 7 new tests, updated VALID_CONFIG and _make_controller |
...
intraday windows. 134 tests pass.
